...
func probeHandler(w http.ResponseWriter, r *http.Request) {
	...
	metricsNamespace := params.Get("namespace")
	...
	if err != nil {
		...
		for _, path := range lookupPaths {
			...
			metricName := strings.ReplaceAll(string(bracketRegex.ReplaceAll([]byte(path[2:]), []byte(""))), ".", "_")
			valueGaugeVec := prometheus.NewGaugeVec(
				prometheus.GaugeOpts{
					Namespace: metricsNamespace,
					Name:      metricName,
					Help:      fmt.Sprintf("Retrieved value by the `%v` JSONPath filter", path),
				},
				labelKeys,
			)
			for idx, labelPath := range labelPaths {
				labelKey := labelKeys[idx]
				res, err := jsonpath.JsonPathLookup(jsonData, labelPath)
				if err != nil {
					http.Error(w, "Jsonpath for label not found", http.StatusNotFound)
					log.Printf("Jsonpath for label(%v) not found: %v", labelKey, jsonData)
					return
				}
			...
			if err != nil {
				...
				log.Printf("Jsonpath(%v) not found: %v", lookupPaths, jsonData)
				return
			}
